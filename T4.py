...
class VAE:
  ...
  def _build_model(self):
    ...
    # s_ELBO = tf.pow(tf.add(-tf.pow(u, tf.reciprocal(alpha)), 1), tf.reciprocal(beta)) # hopefully this works
    z = z_mu + tf.sqrt(tf.exp(z_log_sig_sq)) * epsilon  # [B * M, h_dim]

    # IMPORTANT: sampling the gate value s, on the other hand we do:
    # s ~ Beta(a, b)
    ...
    # We need somehow  to have y appear here somehow somewhere...
    # See T3 for how we do that there. Use the sampled s as gate value.
